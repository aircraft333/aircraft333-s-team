...
> 安全裕量是现有口径下**最大的单一杠杆**：引入裕量比无裕量省 127.9 万元（8.4%），
> 而把「固定 300 kW」换成报童分位规则再省 17.3 万元（1.2%）。
> 它与问题三的调整机制**部分互补**（两者都靠「提前多买」抑制 $5\pi$ 紧急购电），
> 所以叠加后的额外收益（10.2 万元）远小于各自单独生效时的效果。
...
- [ ] **问题三的裕量仍是手调固定 300 kW**，尚未验证能否也换成分位数规则（脚本 `q3_hedge_q.py` 已写好）
