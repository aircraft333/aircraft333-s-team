...
| 三 | `result3.xlsx` | 计划 21,111,088.5 kWh；调整后 21,195,516.5 kWh；紧急 79,074.7 kWh / 41.4 万元；**合计 1,359.8 万元**（报童分位裕量 q=0.80、回看 60 天，日均 169 kW） |
| 四 | `result4-2.xlsx` | 波动电价重算问题二：计划 21,505,541.3 kWh / 1,375.7 万元；紧急 146,514.8 kWh / 87.7 万元；**合计 1,463.4 万元** |
| 四 | `result4-3.xlsx` | 波动电价重算问题三：计划 21,182,990.8 kWh；调整后 21,272,938.7 kWh；紧急 74,875.5 kWh / 42.5 万元；**合计 1,422.4 万元** |
...
| **问题三·主方案（报童分位 q=0.80, win=60）** | **1,297.6** | **20.7** | **41.4** | **1,359.8** |

> 安全裕量是现有口径下**最大的单一杠杆**：引入裕量使问题二省 127.9 万元（8.4%）、
> 问题三省 38.9 万元（2.7%）；两问再换成分位数规则又分别省 17.3 万 / 24.9 万元（1.2% / 1.8%）。
> 两问叠加后问题三比问题二再低 35.1 万元（2.5%）。
...
| 对应问题三 | 13,598,164.7 元 | 14,224,008.1 元 | +625,843.4（+4.60%） |
| 参考：无储能、按现货价买净负荷 | 16,407,319.6 元 | 17,026,782.9 元 | +619,463.3（+3.78%） |

> 波动电价下问题三相对问题二仍省 **41.0 万元（2.80%）**，比固定电价下的 2.52% 还略高。
>
...
- [x] ~~问题三的裕量仍是手调固定 300 kW~~ → 已验证并升级为**报童分位裕量 q=0.80、回看 60 天**
      （`HEDGE_MODE="quantile"`, `HEDGE_PARAM=0.80`, `HEDGE_WIN=60`），全年 **1,359.8 万元**
      （比固定 300 kW 省 24.9 万元）；**年度最优恰好就是报童解析值 1−1/k，零调参**，
