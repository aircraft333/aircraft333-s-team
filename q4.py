...
def main():
    ...
    for nm, c, f in (("对应问题二", c2, fixed["对应问题二"]),
        ...
        log(f"  {nm:<22s} {f:>18,.1f} {c:>18,.1f} {c - f:>+16,.1f} 元"
            f"（{(c / f - 1):+.2%}）")
    log("说明：附件4 在输出窗口（334 天）内的逐时段均价 0.7575 反而**低于**附件1 的 0.7662，")
    log("      但总费用仍高 4.91%。受控分解（见 q4_电价波动效应分解.txt）表明：")
    log("        · 计划购电费 +602,260.3 元中，逐日价格漂移项占 97.2%（+585,294.0 元），")
    log("          逐时段均价项仅 +16,966.3 元；")
    log("        · 根因是 corr(日购电量, 当日电价水平) = +0.9651 —— 负载高（必须多买电）的日子")
    log("          恰好也是电价整体偏高的日子，而储能可用容量 9600 kWh 远不足以跨日搬运")
    log("          （日均购电 6.4 万 kWh），于是价格漂移直接转化为费用上升；")
    log("        · 逐日 LP 相对「照抄固定电价方案」仍省 126,374.6 元，说明模型对波动电价有适应性。")
