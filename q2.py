...
# 实测（当前逐槽被动平衡口径，全年 334 天，见 q2_精细寻优.txt）：
#      hedge =   0 kW 固定      → 15,228,314.3 元（紧急购电 303.3 万元）
#      hedge = 300 kW 固定      → 14,121,654.0 元（紧急购电  63.3 万元）
#      报童分位（q=0.80, win=60）→ 14,001,975.7 元
#      报童分位（q=0.75, win=60）→ 13,983,038.5 元
#      报童分位（q=0.76, win=30）→ 13,949,108.5 元   ← 采用（全年实测最优）
# 参数不敏感性（这是论文里最该强调的一点）：
#      · q   在 0.75~0.78 之间费用差 <0.03%，与报童解析值 0.80 也只差 0.12%；
#      · win 在 30~60 天之间费用差 <0.25%（30 天全年最优，但分半检验显示
#        上半年偏向 30 天、下半年偏向 45 天，即该优势是**季节性**的）。
#      ⇒ 论文应写成「该模型对裕量参数不敏感」而不是「扫出了精确最优」。
...
HEDGE_PARAM = 0.76              # scalar -> 裕量 kW；quantile -> 分位数水平
HEDGE_WIN = 30                  # quantile 模式的回看窗口天数（0 = 全部历史）
