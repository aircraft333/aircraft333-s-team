...
# 安全裕量（对冲 5 倍价紧急购电；报童模型：缺口概率 > 1/5 即值得提前多买）
# 实测（当前逐槽被动平衡口径，全年 334 天）：
#      hedge =   0 kW → 总购电费 15,228,314.3 元（紧急购电 303.3 万元）
#      hedge = 300 kW → 总购电费 14,121,654.0 元（紧急购电  63.3 万元）  ← 省 110.7 万元
# 裕量是当前口径下最大的单一杠杆：逐槽规则下储能被动吸收全部偏差、不会主动留余量，
# 一旦电池提前见底，后续缺口全部按 5π 结算，必须靠「多买一点」把缺口概率压下去。
# 注：q2_参数寻优.txt 是旧口径（储能按计划执行）下跑的，结论不能直接沿用；
#     若要更精细的最优点，请用当前口径重跑 q2_tune.py / q2_hedge.py。
HEDGE_MODE = "scalar"          # "scalar"：全天统一值；"quantile"：逐时段取历史误差分位数
HEDGE_PARAM = 300.0             # scalar -> 裕量 kW；quantile -> 分位数水平
